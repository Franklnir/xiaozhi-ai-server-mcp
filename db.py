...
class Settings(BaseSettings):
    ...
    mcp_supervisor_enabled: bool = Field(
        default=True,
        validation_alias=AliasChoices("MCP_SUPERVISOR_ENABLED", "SCIG_MCP_ENABLED"),
    )
    ...
    allowed_hosts: str = Field(default="", validation_alias=AliasChoices("ALLOWED_HOSTS"))
    csrf_trusted_origins: str = Field(default="", validation_alias=AliasChoices("CSRF_TRUSTED_ORIGINS"))
    ...
    # Password policy
    password_min_length: int = Field(default=8, validation_alias=AliasChoices("PASSWORD_MIN_LENGTH"))
...
def _security_startup_warnings() -> None:
    ...
    if int(settings.password_min_length) < 8:
        logger.warning("[SECURITY] PASSWORD_MIN_LENGTH masih lemah. Disarankan minimal 8-10 karakter.")
    if settings.app_env.lower() == "production":
        if not settings.force_https:
            logger.warning("[SECURITY] APP_ENV=production tapi FORCE_HTTPS=false.")
        if not settings.cookie_secure:
            logger.warning("[SECURITY] APP_ENV=production tapi COOKIE_SECURE=false.")
        if not settings.allowed_hosts.strip():
            logger.warning("[SECURITY] APP_ENV=production tapi ALLOWED_HOSTS belum diisi.")
        if not settings.trust_proxy_headers:
            logger.warning("[SECURITY] APP_ENV=production tapi TRUST_PROXY_HEADERS=false.")
...
CREATE TABLE IF NOT EXISTS user_tracked_devices (
  user_id INT NOT NULL,
  device_id VARCHAR(64) NOT NULL,
  alias VARCHAR(128) NOT NULL,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (user_id, device_id),
  INDEX idx_user_tracked_user (user_id),
  INDEX idx_user_tracked_device (device_id),
  FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS device_status (
  ...
  address_full VARCHAR(512) NULL,
  battery_level FLOAT NULL,
  battery_status VARCHAR(32) NULL,
  charging_type VARCHAR(32) NULL,
  battery_temp FLOAT NULL,
...
CREATE TABLE IF NOT EXISTS device_location_history (
  ...
  INDEX idx_device_location_history_device_time (device_id, recorded_at),
  FOREIGN KEY (device_id) REFERENCES device_registry(device_id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS device_pair_tokens (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  INDEX idx_pair_device (device_id),
...
def db_clear_mcp_code_token(engine: Engine, code_id: int) -> Dict[str, Any]:
    with engine.begin() as conn:
        row = conn.execute(
        ...
        conn.execute(
            text("UPDATE auth_codes SET token='', token_hash=NULL WHERE id=:id"),
            {"id": int(code_id)},
        )
        conn.execute(
            text("DELETE FROM mcp_conn_status WHERE code_id=:id"),
        ...
        return {"ok": True, "code_id": int(code_id), "code": row.get("code"), "used_by": row.get("used_by")}
...
def db_link_tracked_device(engine: Engine, user_id: int, device_id: str, alias: Optional[str] = None) -> None:
    did = normalize_device_id(device_id)
    with engine.begin() as conn:
        if alias is None:
            conn.execute(
                text(
                    """
                    INSERT IGNORE INTO user_tracked_devices (user_id, device_id, alias)
                    VALUES (:u, :d, :a)
                    """
                ),
                {"u": int(user_id), "d": did, "a": did},
            )
        else:
            name = (alias or did).strip() or did
            conn.execute(
                text(
                    """
                    INSERT INTO user_tracked_devices (user_id, device_id, alias)
                    VALUES (:u, :d, :a)
                    ON DUPLICATE KEY UPDATE alias=:a
                    """
                ),
                {"u": int(user_id), "d": did, "a": name},
            )


def db_set_user_device_alias(engine: Engine, user_id: int, device_id: str, alias: str) -> None:
    db_link_tracked_device(engine, user_id, device_id, alias=alias)


def db_get_user_device_alias(engine: Engine, user_id: int, device_id: str) -> Optional[str]:
    did = normalize_device_id(device_id)
    with engine.begin() as conn:
        return conn.execute(
            text("SELECT alias FROM user_tracked_devices WHERE user_id=:u AND device_id=:d"),
            {"u": int(user_id), "d": did},
        ).scalar()


def db_unlink_tracked_device(engine: Engine, user_id: int, device_id: str) -> None:
    did = normalize_device_id(device_id)
    with engine.begin() as conn:
        conn.execute(
            text("DELETE FROM user_tracked_devices WHERE user_id=:u AND device_id=:d"),
            {"u": int(user_id), "d": did},
        )
        ...
        conn.execute(
            text(
                ...
                INSERT INTO device_registry (
                  device_id, owner_user_id, device_name, token_hash, token_prefix,
                  platform, model, os_version
                ) VALUES (
                  :d, :u, :name, :th, :tp, :platform, :model, :osv
                )
            ...
            {
                "d": did,
                "u": int(owner_user_id) if owner_user_id is not None else None,
                "name": device_name,
                "th": token_hash,
                "tp": token_prefix,
                "platform": platform,
...
def db_get_device_registry(engine: Engine, device_id: str) -> Optional[Dict[str, Any]]:
    did = normalize_device_id(device_id)
    with engine.begin() as conn:
        row = conn.execute(
            text(
                """
                SELECT device_id, owner_user_id, device_name, token_hash, token_prefix,
                       platform, model, os_version, last_seen_at, created_at, updated_at
...
def db_get_device_by_token_hash(engine: Engine, token_hash: str) -> Optional[Dict[str, Any]]:
    with engine.begin() as conn:
        row = conn.execute(
            text(
                """
...
def db_update_device_last_seen(engine: Engine, device_id: str) -> None:
    did = normalize_device_id(device_id)
    with engine.begin() as conn:
        conn.execute(
            text("UPDATE device_registry SET last_seen_at=NOW() WHERE device_id=:d"),
            {"d": did},
        )


def db_upsert_device_status(engine: Engine, device_id: str, data: Dict[str, Any]) -> None:
    did = normalize_device_id(device_id)
    with engine.begin() as conn:
        conn.execute(
            text(
                ...
                INSERT INTO device_status (
                  device_id, latitude, longitude, address_street, address_area, address_city,
                  address_full, battery_level, battery_status, charging_type, battery_temp,
                  network_type, signal_strength, carrier, is_online, ram_used, ram_total,
                  storage_used, storage_total
                ) VALUES (
                  :d, :lat, :lon, :street, :area, :city, :full,
                  :bat_level, :bat_status, :chg_type, :bat_temp,
                  :net_type, :sig, :carrier, :online, :ram_used, :ram_total,
                  :storage_used, :storage_total
                )
                ON DUPLICATE KEY UPDATE
                  latitude=:lat,
            ...
            ),
            {
                ...
                "full": data.get("address_full"),
                "bat_level": data.get("battery_level"),
                "bat_status": data.get("battery_status"),
                "chg_type": data.get("charging_type"),
                "bat_temp": data.get("battery_temp"),
                "net_type": data.get("network_type"),
                "sig": data.get("signal_strength"),
                "carrier": data.get("carrier"),
                "online": int(data.get("is_online") or 0),
...
def db_get_device_status(engine: Engine, device_id: str) -> Optional[Dict[str, Any]]:
    did = normalize_device_id(device_id)
    with engine.begin() as conn:
        row = conn.execute(
            text(
                """
                SELECT device_id, latitude, longitude, address_street, address_area, address_city,
                       address_full, battery_level, battery_status, charging_type, battery_temp,
                       network_type, signal_strength, carrier, is_online, ram_used, ram_total,
                       storage_used, storage_total, updated_at
                FROM device_status
                WHERE device_id=:d
                """
            ),
...
def db_add_device_location_history(engine: Engine, device_id: str, data: Dict[str, Any]) -> None:
    did = normalize_device_id(device_id)
    lat = data.get("latitude")
    lon = data.get("longitude")
        ...
        conn.execute(
            text(
                ...
                INSERT INTO device_location_history (
                  device_id, latitude, longitude, address_street, address_area,
...
def db_list_device_location_history(engine: Engine, device_id: str, limit: int = 200) -> List[Dict[str, Any]]:
    did = normalize_device_id(device_id)
    ...
    with engine.begin() as conn:
        rows = conn.execute(
            text(
                """
                SELECT id, device_id, latitude, longitude, address_street, address_area,
                       address_city, address_full, recorded_at
            ...
            {"d": did, "lim": capped_limit},
        ).mappings().all()
        return [dict(r) for r in rows]


def db_list_devices_for_user(engine: Engine, user_id: int) -> List[Dict[str, Any]]:
    with engine.begin() as conn:
        rows = conn.execute(
            text(
                """
                SELECT
                  ...
                  ds.storage_used,
                  ds.storage_total,
                  IF(dr.last_seen_at IS NOT NULL AND TIMESTAMPDIFF(SECOND, dr.last_seen_at, NOW()) <= 30, 1, 0) AS is_online
                FROM user_tracked_devices td
                LEFT JOIN device_registry dr
                  ON dr.device_id = td.device_id
                LEFT JOIN device_status ds
                  ON ds.device_id = td.device_id
                WHERE td.user_id=:u
                ORDER BY dr.last_seen_at DESC, td.updated_at DESC
                """
            ),
            {"u": int(user_id)},
        ).mappings().all()
...
def db_list_devices_admin(engine: Engine, limit: int = 200) -> List[Dict[str, Any]]:
    lim = max(1, min(int(limit), 500))
    with engine.begin() as conn:
        rows = conn.execute(
            text(
                ...
                LEFT JOIN device_status ds ON ds.device_id = dr.device_id
...
def db_create_pair_token(engine: Engine, device_id: str, token_hash: str, expires_at: datetime) -> None:
    did = normalize_device_id(device_id)
    with engine.begin() as conn:
        conn.execute(
            text(
                """
                INSERT INTO device_pair_tokens (token_hash, device_id, expires_at)
                VALUES (:th, :d, :exp)
                ON DUPLICATE KEY UPDATE device_id=:d, expires_at=:exp
                """
            ),
            {"th": token_hash, "d": did, "exp": expires_at},
        )


def db_claim_pair_token(engine: Engine, token_hash: str) -> Optional[str]:
    with engine.begin() as conn:
        row = conn.execute(
            text(
                """
        ...
        if expires_at and expires_at < datetime.utcnow():
            conn.execute(text("DELETE FROM device_pair_tokens WHERE token_hash=:th"), {"th": token_hash})
            return None
        conn.execute(text("DELETE FROM device_pair_tokens WHERE token_hash=:th"), {"th": token_hash})
        return row.get("device_id")


def db_get_geocode_cache(engine: Engine, lat_key: str, lon_key: str) -> Optional[Dict[str, Any]]:
    with engine.begin() as conn:
        row = conn.execute(
            text(
                """
                SELECT address_json, updated_at
                FROM geocode_cache
                WHERE lat_key=:lat AND lon_key=:lon
                """
            ),
            {"lat": lat_key, "lon": lon_key},
        ).mappings().first()
        if not row:
            return None
        try:
            data = json.loads(row.get("address_json") or "{}")
        except Exception:
            data = {}
        return {"data": data, "updated_at": row.get("updated_at")}


def db_set_geocode_cache(engine: Engine, lat_key: str, lon_key: str, address: Dict[str, Any]) -> None:
    payload = json.dumps(address or {}, ensure_ascii=False)
    with engine.begin() as conn:
        conn.execute(
            text(
                """
                INSERT INTO geocode_cache (lat_key, lon_key, address_json)
                VALUES (:lat, :lon, :addr)
                ON DUPLICATE KEY UPDATE address_json=:addr
                """
            ),
            {"lat": lat_key, "lon": lon_key, "addr": payload},
        )
...
def build_role_introduction_for_xiaozhi() -> str:
    return (
        ...
        "6) Jika tool gagal/timeout: jawab sesuai pertanyaan dengan lucu dan nada ekspresi sampai MCP sukses konek kembali.\n"
